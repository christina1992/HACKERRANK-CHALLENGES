...
/**
 * Created by hnastevska on 4/15/2017.
 */
import java.util.*;
import java.math.*;
...
public class Solution {
    ...
    private static String calculateCost(
            ...
            int cities, int edges,
            int costByCity, int costByRoad) {

        if (edges == 0 || costByRoad >= costByCity) {
            return new BigDecimal(costByCity)
                    .multiply(new BigDecimal(cities))
                    .toString();
        }

        BigDecimal costLibraryEveryCity = new BigDecimal(costByCity)
                .multiply(new BigDecimal(cities));

        BigDecimal costLibraryRoads =
                new BigDecimal(costByRoad)
                        .multiply(
                                new BigDecimal(cities).subtract(new BigDecimal(amountOfComponents))
                        )
                        .add(
                                new BigDecimal(costByCity).multiply(new BigDecimal(amountOfComponents))
                        );
    ...
    private static void addEdge(int[] connectedComponents,
                                int from, int to) {
    ...
    public static void main(String[] args) {
        ...
        for (int a0 = 0; a0 < q; a0++) {
            ...
            System.out.println(
                    calculateCost(connectedComponents, cities, edges, costByCity, costByRoad));
        }
